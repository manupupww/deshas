...
def fetch_dollar_bars_cloud(symbol: str, start_date: str, end_date: str, threshold: float = 1_000_000, hf_repo: str = None, hf_token: str = None):
    ...
    # If the file exists but is empty, or doesn't exist, first=True
    file_exists = os.path.exists(output_path)
    if file_exists:
        try:
            # Check if file has data and correct columns
            with open(output_path, 'r') as f:
                first_line = f.readline().strip().split(',')
            
            if len(first_line) != 7 or "timestamp" not in first_line:
                print(f"  [RESET] Bad format or missing headers. Wiping {output_path}")
                os.remove(output_path)
                if os.path.exists(progress_path): os.remove(progress_path)
                if os.path.exists(state_path): os.remove(state_path)
                first = True
                ...
                first = False # Healthy file found, will append without header
        except Exception as e:
            print(f"  [RESET] Error reading header: {e}. Restarting.")
            os.remove(output_path)
            first = True
    else:
        first = True
    ...
    for chunk, current_dt, _ in download_vision_zips(base_url, "aggTrades", clean_symbol, start_dt, end_dt):
        ...
        if not completed_bars_df.empty:
            ...
            bars['timestamp'] = pd.to_datetime(pd.to_numeric(bars['timestamp']), unit='ms').dt.strftime('%Y-%m-%d %H:%M:%S.%f').str[:-3]
...
@app.function(image=image, timeout=86400, cpu=1.0, memory=51200, volumes={"/data": app_volume}, retries=0)
def fetch_volume_bars_cloud(symbol: str, start_date: str, end_date: str, threshold: float = 1000, hf_repo: str = None, hf_token: str = None):
    """Vectorized Volume Bar generation with Persistent Resume logic."""
    print(f"[CLOUD] Volume Bars (Resume): {symbol} | {start_date} -> {end_date} | Threshold: {threshold}")
    clean_symbol = symbol.replace("/", "").replace(":", "")
    base_url = "https://data.binance.vision/data/futures/um"
    start_dt = datetime.strptime(start_date, "%Y-%m-%d")
    end_dt = datetime.strptime(end_date, "%Y-%m-%d")
    
    task_id = f"vbars_v2_{clean_symbol}_{start_date}_{end_date}_{int(threshold)}"
    output_path = f"/data/{task_id}.csv"
    progress_path = f"/data/{task_id}.progress"
    state_path = f"/data/{task_id}.state"
        ...
        if os.path.exists(output_path):
            try:
                ...
                if len(first_line) != 6:
                    ...
                    if os.path.exists(state_path):
                        leftovers = pd.read_csv(state_path)
                    print(f"  [RESUME] Resuming Volume Bars after {last_processed.date()}")
            except Exception as e:
                print(f"  [RESET] Error checking format: {e}. Starting fresh.")
                if os.path.exists(output_path): os.remove(output_path)
        else:
    ...
    if file_exists:
        try:
            ...
            if len(first_line) != 6 or "timestamp" not in first_line:
                print(f"  [RESET] Bad format or missing headers. Wiping {output_path}")
                os.remove(output_path)
                if os.path.exists(progress_path): os.remove(progress_path)
                if os.path.exists(state_path): os.remove(state_path)
    ...
    for chunk, current_dt, _ in download_vision_zips(base_url, "aggTrades", clean_symbol, start_dt, end_dt):
        ...
        chunk['cum_volume'] = chunk['quantity'].cumsum()
        
        chunk['bar_id'] = chunk['cum_volume'] // threshold
        ...
        leftovers = leftovers_df[['timestamp', 'price', 'quantity']].copy()
    ...
    hf_url = None
    if hf_repo and hf_token:
        filename = f"{clean_symbol}_{start_date}_{end_date}_volumeBars_{int(threshold)}.csv"
        success, url_or_err = upload_to_hf(output_path, filename, hf_repo, hf_token)
        if success: hf_url = url_or_err
...
@app.function(image=image, timeout=86400, cpu=1.0, memory=51200, volumes={"/data": app_volume}, retries=0)
def fetch_flow_toxicity_cloud(symbol: str, start_date: str, end_date: str, buckets_per_day: int = 50, window_size: int = 50, hf_repo: str = None, hf_token: str = None):
    """
    Calculates Flow Toxicity (VPIN) and its CDF (Cumulative Distribution Function).
    Uses aggTrades data to create volume buckets and toxicity metrics.
    """
    print(f"[CLOUD] Flow Toxicity CDF: {symbol} | {start_date} -> {end_date} | Buckets/Day: {buckets_per_day} | Window: {window_size}")
    clean_symbol = symbol.replace("/", "").replace(":", "")
    base_url = "https://data.binance.vision/data/futures/um"
    start_dt = datetime.strptime(start_date, "%Y-%m-%d")
    end_dt = datetime.strptime(end_date, "%Y-%m-%d")
    
    task_id = f"ftox_{clean_symbol}_{start_date}_{end_date}_{buckets_per_day}_{window_size}"
    output_path = f"/data/{task_id}.csv"
    
    # 1) Calculate bucket size (Total Volume / Total expected buckets)
    print("  [FT] Calculating total volume for bucket size...")
    total_vol = 0.0
    for chunk, _, _ in download_vision_zips(base_url, "aggTrades", clean_symbol, start_dt, end_dt):
        total_vol += pd.to_numeric(chunk.iloc[:, 2]).sum()
        del chunk
    ...
    bucket_size = total_vol / (days_count * buckets_per_day)
    print(f"  [FT] Bucket Size: {bucket_size:.2f} units | Total Vol: {total_vol:.2f}")
    ...
    for chunk, _, _ in download_vision_zips(base_url, "aggTrades", clean_symbol, start_dt, end_dt):
        ...
        chunk['price'] = pd.to_numeric(chunk['price'])
        chunk['qty'] = pd.to_numeric(chunk['qty'])
        # Binance side: true = buyer is market maker -> Sell trade
        # side=False (buyer is not maker) -> Buy trade
        chunk['is_buy'] = chunk['side'].astype(str).str.strip().str.lower() != 'true'
        ...
        for _, row in chunk.iterrows():
            ...
            if v_vol >= bucket_size:
                buckets.append({
                    ...
                    'buy_vol': v_buy,
                    'sell_vol': v_sell,
                    'oi': abs(v_buy - v_sell) # Absolute Imbalance
                })
                v_ts, v_open, v_high, v_low, v_vol, v_buy, v_sell = None, None, -1.0, float('inf'), 0.0, 0.0, 0.0
    ...
    if not buckets: return {"success": False, "message": "No buckets created."}
    
    # 3) Calculate VPIN and CDF
    df = pd.DataFrame(buckets)
    
    # VPIN formula: sum(|V_buy - V_sell|) / (n * bucket_size) over window n
    df['vpin'] = df['oi'].rolling(window=window_size).sum() / (window_size * bucket_size)
    
    # Drop NaNs from the start of the window
    df = df.dropna(subset=['vpin']).reset_index(drop=True)
    
    if df.empty: return {"success": False, "message": "Not enough data for the specified window size."}
    
    # CDF (Cumulative Distribution Function) -> Percentile rank
    df['vpin_cdf'] = df['vpin'].rank(pct=True)
    
    # Human readable timestamp
    df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms').dt.strftime('%Y-%m-%d %H:%M:%S')
    
    # Keep only target columns
    final_cols = ['timestamp', 'price', 'vpin', 'vpin_cdf']
    df = df[final_cols]
    ...
    hf_url = None
    if hf_repo and hf_token:
        filename = f"{clean_symbol}_{start_date}_{end_date}_FlowToxicity_CDF.csv"
        success, url_or_err = upload_to_hf(output_path, filename, hf_repo, hf_token)
        if success: hf_url = url_or_err
